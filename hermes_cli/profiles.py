...
def create_profile(
    ...
    clone_from:
        Source profile to clone from. Defaults to the currently active profile.
        Use ``"default"`` to clone from ``~/.hermes``.
    clone_all:
        If True, do a full copytree of the source (all state).
        If False and clone_from is set, copy only config files.
    no_alias:
